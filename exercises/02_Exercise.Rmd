...
```{r}
pbtk1cpt.mtc.out <- mcsim("models/pbtk1cpt.model", "inputs/pbtk1cpt.mtc.in")
```


```{r}
head(pbtk1cpt.mtc.out)
```

```{r}
par(mfrow = c(1,4))
hist(pbtk1cpt.mtc.out$Fgutabs, main = "Fgutabs", xlab = "", ylab = "")
hist(pbtk1cpt.mtc.out$kgutabs, main = "kgutabs", xlab = "", ylab = "")
hist(pbtk1cpt.mtc.out$kelim, main = "kelim", xlab = "", ylab = "")
hist(pbtk1cpt.mtc.out$Vdist, main = "Vdist", xlab = "", ylab = "")
```

```{r}
# Find location of the first and last time points by which()
index <- which(names(pbtk1cpt.mtc.out)=="Ccompartment_1.1" | names(pbtk1cpt.mtc.out)=="Ccompartment_1.11")
# Use apply() to find the quantile of each time point
X <- apply(pbtk1cpt.mtc.out[,index[1]:index[2]], 2, quantile, c(0.5, 0.01,0.99))
dat <- t(X)
# Set column names
...
str.pt <- which(names(pbtk1cpt.mtc.out)=="Ccompartment_1.1") 
end.pt <- which(names(pbtk1cpt.mtc.out)=="Ccompartment_1.11") 
par(mfrow = c(1,1))
plot(Theoph$Time, Theoph$conc.uM, pch = "")
for(i in 1:1000){
  lines(t, pbtk1cpt.mtc.out[i,str.pt:end.pt], col="grey")
}
...
```{r}
makemcsim("models/pbtk1cpt.model")
```
...
```{r}
set.seed(1)
mcsim(model = "models/pbtk1cpt.model", input = "inputs/pbtk1cpt.mcmc.in")
```
